...
function buildSummary(avgs: { key: MetricKey; a: number; b: number; diff: number }[], labelA: string, labelB: string): string {
  ...
  const checks: { key: MetricKey; win: string; lose: string }[] = [
    { key: 'auditory',  win: 'stronger vocal delivery',       lose: 'flatter vocal delivery' },
    { key: 'language',  win: 'clearer ideas',                 lose: 'harder to follow language' },
    { key: 'attention', win: 'better sustained focus',        lose: 'more attention drift' },
    { key: 'dmn',       win: 'lower mind-wandering risk',     lose: 'higher mind-wandering risk' },
    { key: 'prosody',   win: 'better rhythm and flow',        lose: 'flatter pacing' },
    { key: 'emotional', win: 'stronger emotional connection', lose: 'less emotional impact' },
    { key: 'memory',    win: 'more memorable content',        lose: 'less memorable content' },
  ];
  for (const c of checks) {
    const m = get(c.key);
    if (Math.abs(m.diff) < 3) continue;
    const winner = win(m.diff, c.key);
    const loser  = winner === labelA ? labelB : labelA;
    parts.push(`${winner} had ${c.win}, while ${loser} showed ${c.lose}.`);
  }
  return parts.join(' ');
}

const INCLUDE_OPTIONS = [
  { key: 'overall',         label: 'Overall comparison' },
  { key: 'vocal',           label: 'Vocal delivery breakdown' },
  { key: 'emotional',       label: 'Emotional impact' },
  { key: 'memory',          label: 'Memory & retention' },
  { key: 'recommendations', label: 'Actionable recommendations' },
  { key: 'executive',       label: 'Executive summary' },
];

function buildPrompt(
  avgs: { key: MetricKey; a: number; b: number; diff: number }[],
  labelA: string,
  labelB: string,
  choices: { audience: string; tone: string; length: string; include: string[] },
  extra: string
): string {
  const rows = avgs.map(m => `  ${m.key}: ${labelA}=${m.a}, ${labelB}=${m.b}, diff=${m.diff > 0 ? '+' : ''}${m.diff}`).join('\n');
  const sections = choices.include.map(k => INCLUDE_OPTIONS.find(o => o.key === k)?.label ?? k).join(', ');

  return `I have neural engagement data from two speeches analysed using the Tribe v2 fMRI brain encoding model. Please write ${choices.length} for ${choices.audience} in a ${choices.tone} tone.
...
Important: DMN is the opposite of all others — lower DMN = better (less mind-wandering).

Metric explanations:
- Engagement: overall attention held by the audience
- Auditory: how strongly the brain responded to the voice — tone, clarity, variation
- Language: how easily the brain decoded the words — clarity vs complexity
- Attention: sustained mental focus throughout
- DMN: mind-wandering — lower is better
- Prosody: rhythm, flow, and natural intonation
- Emotional: emotional resonance and personal connection
- Memory: how likely the content is to be remembered

Please include: ${sections}.${extra.trim() ? `\n\nAdditional instructions: ${extra.trim()}` : ''}`;
}
...
function CompareSummary({ dataA, dataB, labelA, labelB }: { dataA: ChartData; dataB: ChartData; labelA: string; labelB: string }) {
  ...
  const [choices, setChoices] = useState({
    audience: 'a client',
    tone:     'professional',
    length:   'a detailed 3-section report',
    include:  ['overall', 'vocal', 'emotional', 'memory', 'recommendations', 'executive'] as string[],
  });
  ...
  function toggleInclude(key: string) {
    setChoices(c => ({
      ...c,
      include: c.include.includes(key) ? c.include.filter(k => k !== key) : [...c.include, key],
    }));
  }

  const prompt = buildPrompt(avgs, labelA, labelB, choices, extra);
