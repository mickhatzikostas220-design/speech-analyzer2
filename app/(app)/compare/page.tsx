...
function openPrintWindow(report: string, labelA: string, labelB: string) {
  const html = `<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8" />
  <title>Speech Comparison Report — ${labelA} vs ${labelB}</title>
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    body { font-family: Georgia, serif; color: #1a1a1a; padding: 48px; max-width: 720px; margin: 0 auto; line-height: 1.7; }
    h1 { font-size: 22px; font-weight: bold; margin-bottom: 4px; }
    .subtitle { color: #555; font-size: 13px; margin-bottom: 32px; }
    .report { font-size: 15px; white-space: pre-wrap; }
    .footer { margin-top: 48px; padding-top: 16px; border-top: 1px solid #ddd; font-size: 11px; color: #999; }
  </style>
</head>
<body>
  <h1>Speech Comparison Report</h1>
  <p class="subtitle">${labelA} vs ${labelB} &nbsp;·&nbsp; Generated by ACA &nbsp;·&nbsp; ${new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
  <div class="report">${report.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</div>
  <div class="footer">Powered by Tribe v2 fMRI brain encoding model · ACA Neural Speech Analysis</div>
  ...
  async function handleGenerate() {
    ...
    try {
      const res = await fetch('/api/compare-report', {
        ...
        body: JSON.stringify({ labelA, labelB, avgs, choices: { ...choices, include: choices.include.map(k => INCLUDE_OPTIONS.find(o => o.key === k)?.label ?? k) } }),
      });
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 space-y-5">
      ...
      <div className="border-t border-zinc-800 pt-5 space-y-4">
        <h2 className="text-sm font-medium text-zinc-300">Generate Report</h2>
        ...
        <button onClick={handleGenerate} disabled={generating || choices.include.length === 0}
      ...
      {report && (
        <div className="border-t border-zinc-800 pt-5 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => openPrintWindow(report, labelA, labelB)}
          ...
          <div className="bg-zinc-950 border border-zinc-800 rounded-lg p-4">
            <p className="text-sm text-zinc-300 leading-relaxed whitespace-pre-wrap">{report}</p>
          </div>
        </div>
      )}
    </div>
