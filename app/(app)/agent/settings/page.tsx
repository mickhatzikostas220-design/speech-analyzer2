    ...
    apps: [
      {
        ...
        label: 'X / Twitter',
        icon: 'X',
        description: 'Analytics (followers, engagement) · Post tweets — requires OAuth 2.0 user token with tweet.write scope for posting',
        keyLabel: 'OAuth 2.0 user access token (tweet.write) — or Bearer token for read-only',
        keyPlaceholder: 'AAAAAAAAAAAAAAAA…',
        helpText: 'For posting: developer.twitter.com → OAuth 2.0 → User Access Token (tweet.write). For analytics only: Bearer Token under Keys & Tokens.',
      },
      {
        type: 'apikey',
        id: 'instagram',
        label: 'Instagram',
        icon: 'IG',
        description: 'Analytics (followers, reach, post stats) · Post photos — Business/Creator account required',
        keyLabel: 'Graph API long-lived access token (instagram_content_publish for posting)',
        keyPlaceholder: 'EAAGm0…',
        helpText: 'developers.facebook.com/tools/explorer → select your app → add instagram_content_publish + pages_read_engagement permissions → Generate.',
      },
      {
        type: 'apikey',
        id: 'youtube',
        label: 'YouTube',
        icon: 'YT',
        description: 'Channel subscribers, views, video count (read-only)',
        keyLabel: 'YouTube Data API v3 key',
        keyPlaceholder: 'AIzaSy…',
        helpText: 'console.cloud.google.com → APIs → YouTube Data API v3 → Credentials → Create API key.',
      },
      {
        type: 'apikey',
        id: 'linkedin',
        label: 'LinkedIn',
        icon: 'in',
        description: 'Profile info · Share posts — requires w_member_social scope for posting',
        keyLabel: 'OAuth access token (r_liteprofile + w_member_social for posting)',
        keyPlaceholder: 'AQV…',
        helpText: 'linkedin.com/developers → My Apps → Products → enable "Share on LinkedIn" → OAuth 2.0 → request r_liteprofile + w_member_social scopes.',
      },
      {
        type: 'apikey',
        id: 'facebook',
        ...
        icon: 'f',
        description: 'Page analytics (fans, followers) · Post to Page — requires pages_manage_posts scope',
        keyLabel: 'Page access token (pages_manage_posts for posting)',
        keyPlaceholder: 'EAAGm0…',
        helpText: 'developers.facebook.com/tools/explorer → select your Page → add pages_manage_posts + pages_read_engagement → Generate Page Access Token.',
      },
      {
        type: 'apikey',
        id: 'tiktok',
        label: 'TikTok',
        icon: 'TT',
        description: 'Analytics (followers, likes, video count) · Post videos — requires video.publish scope',
        keyLabel: 'TikTok OAuth access token (video.publish for posting)',
        keyPlaceholder: 'act.…',
        helpText: 'developers.tiktok.com → Manage Apps → enable video.publish scope → OAuth token. Videos must be hosted at a public URL for upload.',
      },
    ],
  },
  {
