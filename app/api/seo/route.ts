...
import { safeFetch, isBlockedHost } from '@/lib/net/safeFetch';
...
async function fetchHtml(url: string): Promise<{ html: string; xRobotsTag: string }> {
  ...
  try {
    // safeFetch blocks private/internal hosts and re-validates every redirect
    // hop (a public host can't 302 us to the metadata service).
