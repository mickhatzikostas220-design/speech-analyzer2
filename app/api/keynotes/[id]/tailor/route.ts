...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  // Pull in who this speaker is — Brand Kit + memory — so the re-frame keeps
  // their authentic voice and priorities rather than a generic industry rewrite.
  const speakerContext = (await getPersonaContext(supabase, user.id)) || undefined;
