...
// Escape any value that ends up in the HTML response. `req.name` / `req.email`
// are attacker-supplied at request-access time, so interpolating them raw would
// be a stored-XSS vector in the admin's browser.
...
export async function GET(request: NextRequest) {
  ...
  if (payload.action === 'approve') {
    ...
    if (linkError) return htmlPage('Error', `Failed to generate invite: ${esc(linkError.message)}`, '#ef4444');
