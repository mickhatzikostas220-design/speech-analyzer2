...
import { safeFetch, BlockedUrlError } from '../ssrf';
...
async function fetchHtml(url: string): Promise<{ html: string; finalUrl: string }> {
  ...
  try {
    // safeFetch blocks internal/private hosts (and re-checks every redirect hop)
    // so a speaker's "website URL" can't be pointed at cloud metadata / localhost.
