...
// Escape untrusted values before interpolating them into email HTML. The
// access-request form is public, so name/email/reason are attacker-controlled
// and must never be able to inject markup into the admin's inbox.
...
export async function sendAccessRequestNotification(
  ...
  await resend.emails.send({
    from: FROM,
    to: ADMIN_EMAIL,
    subject: `New access request from ${esc(name)}`,
    html: `
      <div style="font-family:system-ui,sans-serif;max-width:520px;margin:0 auto;padding:32px 24px;background:#09090b;color:#fafafa;border-radius:16px;">
        ...
        <table style="width:100%;border-collapse:collapse;margin-bottom:24px;">
          <tr>
            ...
            <td style="padding:8px 0;color:#fafafa;font-size:13px;font-weight:500;">${esc(name)}</td>
          </tr>
          <tr>
            <td style="padding:8px 0;color:#71717a;font-size:13px;">Email</td>
            <td style="padding:8px 0;color:#fafafa;font-size:13px;">${esc(email)}</td>
          </tr>
          <tr>
            <td style="padding:8px 0;color:#71717a;font-size:13px;vertical-align:top;">Reason</td>
            <td style="padding:8px 0;color:#fafafa;font-size:13px;line-height:1.5;">${esc(reason)}</td>
          </tr>
        </table>
  ...
  await resend.emails.send({
    ...
    html: `
      <div style="font-family:system-ui,sans-serif;max-width:480px;margin:0 auto;padding:32px 24px;background:#09090b;color:#fafafa;border-radius:16px;">
        <div style="width:40px;height:40px;background:linear-gradient(135deg,#a855f7,#6366f1);border-radius:10px;margin-bottom:20px;"></div>
        <h1 style="font-size:20px;font-weight:600;margin:0 0 8px;">You're in, ${esc(name)}.</h1>
  ...
  await resend.emails.send({
    ...
    html: `
      <div style="font-family:system-ui,sans-serif;max-width:480px;margin:0 auto;padding:32px 24px;background:#09090b;color:#fafafa;border-radius:16px;">
        ...
        <p style="color:#a1a1aa;margin:0 0 16px;line-height:1.6;">Hi ${esc(name)}, we reviewed your request for access to ACA but are unable to approve it at this time.</p>
