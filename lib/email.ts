...
// Escape user-supplied values before interpolating them into HTML emails, so a
// requester can't inject markup/links into the admin notification (or their own
...
export async function sendAccessRequestNotification(
  ...
  await resend.emails.send({
    from: FROM,
    to: ADMIN_EMAIL,
    subject: `New access request from ${safeName}`,
    html: `
