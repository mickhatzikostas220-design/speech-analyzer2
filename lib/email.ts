  ...
  await resend.emails.send({
    ...
    html: `
      <div style="font-family:system-ui,sans-serif;max-width:520px;margin:0 auto;padding:32px 24px;background:#09090b;color:#fafafa;border-radius:16px;">
        ...
        <p style="color:#71717a;font-size:13px;margin:0 0 24px;">Someone wants access to Orator.</p>

        <table style="width:100%;border-collapse:collapse;margin-bottom:24px;">
          <tr>
            <td style="padding:8px 0;color:#71717a;font-size:13px;width:80px;">Name</td>
            <td style="padding:8px 0;color:#fafafa;font-size:13px;font-weight:500;">${escapeHtml(name)}</td>
          </tr>
          <tr>
            <td style="padding:8px 0;color:#71717a;font-size:13px;">Email</td>
            <td style="padding:8px 0;color:#fafafa;font-size:13px;">${escapeHtml(email)}</td>
          </tr>
          <tr>
            <td style="padding:8px 0;color:#71717a;font-size:13px;vertical-align:top;">Reason</td>
            <td style="padding:8px 0;color:#fafafa;font-size:13px;line-height:1.5;">${escapeHtml(reason)}</td>
          </tr>
        </table>
        ...
        <p style="color:#3f3f46;font-size:11px;margin-top:28px;">Links expire in 7 days. Orator · Neural Speech Analysis</p>
      </div>
    `,
  });
}
  ...
  await resend.emails.send({
    ...
    html: `
      <div style="font-family:system-ui,sans-serif;max-width:480px;margin:0 auto;padding:32px 24px;background:#09090b;color:#fafafa;border-radius:16px;">
        ...
        <p style="color:#a1a1aa;margin:0 0 16px;line-height:1.6;">Hi ${escapeHtml(name)}, we reviewed your request for access to Orator but are unable to approve it at this time.</p>
