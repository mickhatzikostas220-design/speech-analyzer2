 ...
 * claim only that user's jobs — used when a signed-in user flushes their own
 * queue, so one user can never drain (or trigger work for) another user's jobs.
 * The cron drain omits it to process the whole queue.
 */
