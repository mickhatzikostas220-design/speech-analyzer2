 ...
 * claim only that user's jobs — used when a signed-in user triggers a flush of
 * their own queue, so they can't drain (and force-run) other users' jobs. The
 * cron runner omits it to drain the whole queue.
 */
